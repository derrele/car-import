...
export interface ScoreWeights { roi:number; saleSpeed:number; absoluteProfit:number; risk:number; marketPrice:number; capitalRequired:number; }
export interface AnalysisInput { purchasePrice: number; expectedSalePrice: number; costs: ImportCosts; holdingDays: number; annualCapitalCostRate: number; targetRoi: number; scoreWeights?:ScoreWeights; }
